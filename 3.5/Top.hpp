...
SC_MODULE(Top)
{
    ...
    sc_fifo<sc_dt::sc_uint<DATA_BITS>> fifo;
    ...
    SC_CTOR(Top) : clock("clock", CLK_PERIODE, SC_NS), adapter("adapter"), slave("slave"), master("master"), fifo("fifo", FIFO_SIZE)
    {
        master.clk(clock);
        master.data_out(fifo);
